...
from app.services.progress import complete_lesson_and_advance
...
LEGACY_MANUAL_TYPES = frozenset({"resource_exists", "navigation", "job_completed"})


def is_manual_verify_type(verify_type: str) -> bool:
    return verify_type == "manual" or verify_type in LEGACY_MANUAL_TYPES


class _VerifiableState(Protocol):
    status: str
    started_at: datetime | None


def ensure_active_or_locked_response(
    db: Session,
    state: _VerifiableState,
    *,
    locked_message: str,
    hint_key: str,
    hint_id: str | None,
