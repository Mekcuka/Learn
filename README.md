...
**Проверка статуса** (из корня репозитория):
...
Сервис `postgres` должен быть в состоянии `running`; в выводе `Test-NetConnection` — `TcpTestSucceeded : True` (порт **5433** на хосте).

**Симптом:** `connection refused` на `localhost:5433` — контейнер не запущен или порт занят другим процессом.

**Запуск** (если не running):
...
Повторите `Test-NetConnection` или `ps` — убедитесь, что `postgres` в `running`.

Контейнер слушает **5433** (`5433:5432`); `DATABASE_URL` в `backend/.env.example` — `postgresql://learn:learn@localhost:5433/learn`.

**Первый запуск БД** — после старта контейнера обязательно выполните миграции (шаг «2. Backend»): `alembic upgrade head`.
...
PostgreSQL запущен (`docker compose -f deploy/docker-compose.yml ps` или `Test-NetConnection localhost -Port 5433`), миграции применены, seed выполнен — два терминала. Если БД не отвечает — см. раздел [«1. PostgreSQL»](#1-postgresql).
...
| Backend не подключается к БД, `connection refused` на `:5433` | Запустите PostgreSQL: `docker compose -f deploy/docker-compose.yml up -d postgres`; проверьте `ps` / `Test-NetConnection localhost -Port 5433`; при первом запуске — `alembic upgrade head` |
