...
import { lessonLayoutGridClasses, LESSON_BODY_GRID, LESSON_HEADER_GRID } from "./lessonLayout";
...
describe("lessonLayoutGridClasses", () => {
  ...
  it("returns 2-column classes when hints column is hidden", () => {
    expect(lessonLayoutGridClasses(false)).toEqual({
      headerGrid: LESSON_HEADER_GRID.noHints,
      body: LESSON_BODY_GRID.noHints,
    });
  });
});
